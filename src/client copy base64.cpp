...
int main(int ac, char* av[]) {
  ...
  try {
    ...
    mesh.onReceive([&mesh](uint32_t nodeId, std::string& msg) {
      ...
      try
      {
        std::string encodedImage = msg.substr(4); // Убираем "IMG:"
        std::string decoded = base64_decode(encodedImage);
        ...
        std::cout << "Time to process and send frame: " << duration.count() << " ms\n";
      }
