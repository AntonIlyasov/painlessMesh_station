...
void encodeAndSendImage(const cv::Mat &frame) {
  ...
  cv::imencode(".jpg", frame, imgImencode, std::vector<int> {cv::IMWRITE_JPEG_QUALITY, 100});  
  std::string encoded = base64_encode(imgImencode.data(), imgImencode.size());
  std::string msg = "IMG:" + encoded;
  std::cout << "send sendSingle(2, msg) = " << msg.length() << "\n";
  // mesh.sendBroadcast(msg);
  mesh.sendSingle(2, msg);
}
